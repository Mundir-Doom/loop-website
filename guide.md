# Loop WebGL Menus Guide

## Overview
- Single-page Vite + React site pairing the ReactBits Infinite Menu hero canvas with the Bubble Menu navigation.
- Splash overlay fades in with `logo2.svg`, now slightly larger for emphasis, before revealing the interactive menus (Bubble Menu still uses `logo1.svg`).
- Inline dark background styles eliminate the initial white flash during app boot.
- Core deps: `gl-matrix` (Infinite Menu math) and `gsap` (Bubble Menu animation); Vite handles bundling.
...
## Project layout
- `index.html` sets a dark inline background to prevent white flashes and mounts the React tree into `#root`.
- `src/main.tsx` boots the React app.
- `src/index.css` defines theme colours plus the logo intro styling (larger logo) and layout helpers.
- `src/vite-env.d.ts` enables SVG imports via Vite typings.
- `src/App.tsx` orchestrates the logo intro overlay, Bubble Menu, and Infinite Menu with demo data.
- `src/components/InfiniteMenu.*` holds the WebGL-based menu implementation and styling.
- `src/components/BubbleMenu.*` supplies the GSAP-powered navigation and styling.

## Customise the experience
**Intro logo overlay**
- Swap `logo2.svg` with your own asset in `App.tsx` if needed.
- Adjust the hold duration (`1200` ms), fade timing (`700` ms), or `.intro-logo` clamp sizes to change pacing and scale.
- Bubble menu logo remains `logo1.svg`; update its import separately if you want everything unified.
- Tweak `.intro-overlay` background to match brand colours.

**Infinite Menu**
- Edit `infiniteMenuItems` in `App.tsx` to change hero tiles and links.
- Ensure remote images allow CORS; host locally via `import` for guaranteed texture loading.
- Tune motion via constants (e.g., `ROTATION_ACCELERATION`) inside `InfiniteMenu.tsx` and match overlays in its CSS.
- Update overlay markup/text near the component bottom and adjust `InfiniteMenu.css` selectors accordingly.

**Bubble Menu**
- Adjust `bubbleMenuItems` entries for real routes, hover colours, and rotation offsets.
- Toggle `useFixedPosition` or tweak GSAP timing props (`animationEase`, `animationDuration`, `staggerDelay`) in `App.tsx` for different entrance behaviour.
- Edit pill styles in `BubbleMenu.css` to change spacing, responsive breakpoints, or hover treatments.

## Production notes
- Preload hero images (or use low-quality placeholders) for smoother Infinite Menu texture transitions.
- Keep Bubble Menu labels concise so pills remain legible on narrow viewports; adjust media queries as required.
- Provide a non-WebGL fallback (static grid or poster) for browsers lacking WebGL2 support.
- Run Lighthouse after swapping production assets—large textures and motion can affect first paint.
...
### `src/components/InfiniteMenu.css`
```css
